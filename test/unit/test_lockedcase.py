...
class LockedCaseTest(unittest.TestCase):
    ...
    def test_create_new_role(self):
        logging.info("LockedCaseTest.test_create_new_role")
        case = LockedCase()
        region_name = "sandbox"
        role_name = "QQQ"
        region_x = 1
        region_y = 3
        region_finded = False
        for re in self.default_world.region:
            if re.name == region_name:
                region_finded = True
                len_of_role = len(re.role)
                case.createNewRole(self.default_world, region_name, role_name, region_x, region_y)
                self.assertTrue(len(re.role) == len_of_role+1)
                break
        self.assertTrue(region_finded)
