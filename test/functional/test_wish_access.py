...
class WishAccessTest(unittest.TestCase):
    ...
    def test_get_dream_data(self):
        ...
        self.assertEqual(realizer_uuid, res_m.getContents()["dream_realizer_uuid_list"])
    ...
    def test_make_a_promise(self):
        ...
        dream_uuid = str(uuid.uuid1())
        dreamer_uuid = str(uuid.uuid1())
        realizer_uuid_1 = str(uuid.uuid1())
        realizer_uuid_2 = str(uuid.uuid1())
        db = SQLite3Db()
        db.insertOne("dream", [dream_uuid, dreamer_uuid, None, 1, "test_make_a_promise", "", "", 10, "sandbox", "muchu"])
        #進行測試
        req_m_1 = Message("make_a_promise", {"dream_uuid":dream_uuid, "realizer_uuid":realizer_uuid_1})
        req_m_2 = Message("make_a_promise", {"dream_uuid":dream_uuid, "realizer_uuid":realizer_uuid_2})
        res_m_1 = self.default_cli.sendMessage(req_m_1)
        res_m_2 = self.default_cli.sendMessage(req_m_2)
        self.assertTrue(res_m_1.isValid())
        self.assertTrue(res_m_2.isValid())
        self.assertEqual("make_a_promise", res_m_1.getTitle())
        self.assertEqual("make_a_promise", res_m_2.getTitle())
        dreamData = db.selectSpecify("dream", {"dreamUUID":dream_uuid})
        self.assertEqual(1, len(dreamData))
        self.assertEqual(2, dreamData[0][3]) #檢查狀態碼 1->2
        self.assertTrue(realizer_uuid_1 in dreamData[0][2].split(",")) #檢查 實現者1號 UUID
        self.assertTrue(realizer_uuid_2 in dreamData[0][2].split(",")) #檢查 實現者2號 UUID
