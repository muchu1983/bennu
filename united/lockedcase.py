...
class LockedCase:
    ...
    def realizerMakeAPromise(self, dream_uuid, realizer_uuid):
        ...
        if dreamData != None:
            realizer_uuid_list = None
            if dreamData[0][2] == None:
                realizer_uuid_list = [realizer_uuid]
            else:
                realizer_uuid_list = dreamData[0][2].split(",")
                if realizer_uuid not in realizer_uuid_list:
                    realizer_uuid_list.append(realizer_uuid)
            db.updateSpecify("dream", {"realizerUUID":",".join(realizer_uuid_list), "state":2}, {"dreamUUID":dream_uuid})
        self.getLock().release()
        return 0
    ...
    def realizerAbortDreamPromise(self, dream_uuid, realizer_uuid):
        ...
        if dreamData != None:
            realizer_uuid_list = dreamData[0][2].split(",")
            if realizer_uuid in realizer_uuid_list:
                realizer_uuid_list.remove(realizer_uuid)
            if len(realizer_uuid_list) == 0:
                stateCode = 1
            else:
                stateCode = 2
            rData = ",".join(realizer_uuid_list)
            if rData == "": rData = None
        db.updateSpecify("dream", {"realizerUUID":rData, "state":stateCode}, {"dreamUUID":dream_uuid})
        self.getLock().release()
        return 0
    ...
    def realizerAwardWinning(self, world, dream_uuid, realizer_uuid):
        ...
        db = SQLite3Db()
        dreamData = db.selectSpecify("dream", {"dreamUUID":dream_uuid})
        if dreamData != None and realizer_uuid in dreamData[0][2].split(","):
            #award 更新
            ...
            realizer_uuid_list.remove(realizer_uuid)
            if len(realizer_uuid_list) == 0:
                #所有 realizer 均已領取 award => 刪除 dream
                ...
                #尚有 realizer 未領取 award
                db.updateSpecify("dream", {"realizerUUID":",".join(realizer_uuid_list)}, {"dreamUUID":dream_uuid})
        self.getLock().release()
        return 0
    ...
    def getDreamData(self, dream_uuid):
        ...
        if len(dreamData) == 1:
            ...
            ret["dream_realizer_uuid_list"] = dreamData[0][2]
